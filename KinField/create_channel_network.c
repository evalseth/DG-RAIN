...
void create_channel_network(char *ChannelCoordinates, char *JunctionCoordinates)
{
	...
	for (int i = 0; i < NumChannels; i++)
	{
		...
		ChannelList[i]->BCvals = xcalloc(4,sizeof(int));
	...
	// initialize BCtype to -1
		...
		if (bc0 == 1 || bc0 ==2)
		{
			...
			sscanf(g2, "%lf", &val);
			if (bc0 ==1)
				ChannelList[i]->BCvals[0] = val;
			else
				ChannelList[i]->BCvals[1] = val;
		}
		...
		else if (bc0 == 3)
		{
			...
			ChannelList[i]->BCvals[0] = val1;
			ChannelList[i]->BCvals[1] = val2;
		}
		...
		if (bc1 == 1 || bc1 ==2)
		{
			...
			sscanf(g2, "%lf", &val);
			if (bc0 ==1)
				ChannelList[i]->BCvals[2] = val;
			else
				ChannelList[i]->BCvals[3] = val;
		}
		...
		else if (bc1 == 3)
		{
			...
			double val1, val2;
			sscanf(g2, "%lf %lf", &val1, &val2);
			ChannelList[i]->BCvals[2] = val1;
			ChannelList[i]->BCvals[3] = val2;
		}
	...
	for (int i = 0; i < NumJunctions; i++)
	{
		...
		//JunctionList[i]->ElCount = xcalloc(JunctionList[i]->NumVerts, sizeof(int));
		//int EdgCount = createEdgtoVertAndElCount(JunctionList[i]->EltoVert, JunctionList[i]->NumEl, JunctionList[i]->NumVerts, JunctionList[i]->EdgtoVert, JunctionList[i]->ElCount);
		//if (EdgCount != JunctionList[i]->TotalNumEdges)	
		//{
		//	printf("EdgCount = %d\n", EdgCount);
