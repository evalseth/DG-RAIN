...
void calculateMassMatrix1D(int Np, gsl_matrix* V, gsl_matrix** MassMatrix)
{
	*MassMatrix = gsl_matrix_alloc(Np,Np);
	InvM = gsl_matrix_alloc(Np,Np);
	gsl_blas_dgemm(CblasTrans, CblasNoTrans, 1.0, V, V, 0.0, InvM);
...
void preprocess_basis_functions()
{
	int P = 1;
	int Np = P+1;
	...
	{
		KinRouteList[i]->P = P;
		KinRouteList[i]->Np = Np;
		int NumEl = KinRouteList[i]->NumEl;
		int NumNodes = Np*KinRouteList[i]->NumEl;
		KinRouteList[i]->NumNodes = NumNodes;
		KinRouteList[i]->NodalX = xcalloc(NumNodes, sizeof(double));
		KinRouteList[i]->NodalY = xcalloc(NumNodes, sizeof(double));
		KinRouteList[i]->NodalZ = xcalloc(NumNodes, sizeof(double));
		KinRouteList[i]->NodalnFriction = xcalloc(NumNodes, sizeof(double));
		KinRouteList[i]->dz = xcalloc(NumNodes, sizeof(double));
	
	calculateNodalCoordinates(P, NumEl, KinRouteList[i]->x, KinRouteList[i]->NodalX);
		calculateNodalCoordinates(P, NumEl, KinRouteList[i]->y, KinRouteList[i]->NodalY);
		InterpolateWithGSLSplines(KinRouteList[i]->NumEdges, NumNodes, KinRouteList[i]->x, KinRouteList[i]->y, KinRouteList[i]->NodalX, KinRouteList[i]->NodalY, KinRouteList[i]->z, KinRouteList[i]->NodalZ, KinRouteList[i]->dz);
		InterpolateWithGSLSplines(KinRouteList[i]->NumEdges, NumNodes, KinRouteList[i]->x, KinRouteList[i]->y, KinRouteList[i]->NodalX, KinRouteList[i]->NodalY, KinRouteList[i]->nFriction, KinRouteList[i]->NodalnFriction, NULL);
	...
	Lift1D(Np, V, &LIFT);
	calculateMassMatrix1D(Np, V,&MassMatrix);
